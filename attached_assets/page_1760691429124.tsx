...
export default function PrijzenCalculatorPage() {
  ...
  // Genereer de URL voor de "Nu bestellen" knop
  const generateOrderUrl = () => {
    ...
    params.set("puntentelling", puntentelling.toString())
    params.set("adviesrapport", adviesrapport.toString())
    params.set("fotografie", fotografie.toString())
    params.set("spoedService", spoedService.toString())
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F7]">
      ...
      <header className="relative z-30 px-4 md:px-6 py-4 md:py-6 bg-transparent">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center space-y-3 md:space-y-4">
            {/* Brand Name Above Navigation - Make it clickable */}
            <Link href="/" className="inline-block">
              <div className="text-white font-bold text-base md:text-lg tracking-widest hover:text-lime-300 transition-colors cursor-pointer">
                Label & Lens
              </div>
            </Link>
            ...
            <nav
              className="flex items-center space-x-6 md:space-x-12 text-sm md:text-base"
              role="navigation"
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <Card className="border-2 border-lime-200 shadow-2xl rounded-2xl bg-white w-full">
              <CardHeader className="bg-lime-50 py-6 rounded-t-2xl text-center">
              ...
              <CardContent className="space-y-6 md:space-y-8 p-4 md:p-8">
                ...
                <div className="space-y-4 md:space-y-6">
                  <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-3 md:mb-4">Stap 1: Woningtype</h3>
                  <div className="grid grid-cols-2 gap-4 md:gap-6">
                    <button
                      onClick={() => setWoningType("koop")}
                      className={`p-4 md:p-6 border-2 rounded-xl font-medium transition-all duration-300 ${
                        woningType === "koop"
                    ...
                    >
                      <Home className="w-6 md:w-8 h-6 md:h-8 mx-auto mb-2 md:mb-3 text-current" />
                      <div className="text-base md:text-lg font-semibold">Koopwoning</div>
                      <div className="text-xs md:text-sm mt-1 md:mt-2 opacity-75">
                        Inclusief NEN 2580 meting & plattegrond
                      </div>
                    </button>
                    <button
                    ...
                    >
                      <Home className="w-6 md:w-8 h-6 md:h-8 mx-auto mb-2 md:mb-3 text-current" />
                      <div className="text-base md:text-lg font-semibold">Huurwoning</div>
                      <div className="text-xs md:text-sm mt-1 md:mt-2 opacity-75">Energielabel & plattegrond</div>
                    </button>
                  </div>
                </div>
                ...
                {woningType && (
                  <div className="space-y-4 md:space-y-6">
                    <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-3 md:mb-4">Stap 2: Oppervlakte</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
                      {[
                        ...
                        <button
                          ...
                          className={`p-3 md:p-4 border-2 rounded-lg font-medium transition-all duration-300 ${
                        ...
                        >
                          ...
                          <div className="text-xs md:text-sm text-gray-600 mt-1">€{option.price}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {woningType === "huur" && oppervlakte && (
                  <div className="space-y-4 md:space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      ...
                      >
                        <div className="text-base md:text-lg font-semibold">Puntentelling</div>
                        <div className="text-sm mt-1 md:mt-2 opacity-75">+€120</div>
                      </button>
                      <button
                      ...
                      >
                        <div className="text-base md:text-lg font-semibold">Zonder puntentelling</div>
                      </button>
                    </div>
                  </div>
                )}

                {/* Stap 4: Adviesrapport - alleen voor huurwoningen */}
                {woningType === "huur" && oppervlakte && (
                  <div className="space-y-4 md:space-y-6">
                    <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-3 md:mb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      ...
                      >
                        <div className="text-base md:text-lg font-semibold">Adviesrapport</div>
                        <div className="text-xs md:text-sm mt-1 md:mt-2 opacity-75">indien nodig</div>
                        <div className="text-xs md:text-sm mt-1 opacity-75">+€100</div>
                      </button>
                      <button
                      ...
                      >
                        <div className="text-base md:text-lg font-semibold">Geen adviesrapport</div>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {oppervlakte && (
                  <div className="space-y-4 md:space-y-6">
                    ...
                    <div className="space-y-4">
                      {/* Fotografie */}
                      <label
                        className={`flex items-center p-3 md:p-4 border-2 rounded-lg cursor-pointer transition-all duration-300 ${
                          fotografie
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <Camera className="w-5 md:w-6 h-5 md:h-6 text-purple-500 mr-3 md:mr-4 flex-shrink-0" />
                        <div className="flex-1">
                          <div className="font-semibold text-gray-800 text-sm md:text-base">
                            Professionele Fotografie
                          </div>
                          <div className="text-xs md:text-sm text-gray-600">Hoogwaardige vastgoedfoto's</div>
                        </div>
                        <div className="text-base md:text-lg font-bold text-purple-600">+€150</div>
                      </label>
                      ...
                      >
                        <input
                          ...
                          className="sr-only"
                        />
                        <Zap className="w-5 md:w-6 h-5 md:h-6 text-red-500 mr-3 md:mr-4 flex-shrink-0" />
                        <div className="flex-1">
                          <div className="font-semibold text-gray-800 text-sm md:text-base">Spoed Service</div>
                          <div className="text-xs md:text-sm text-gray-600">Binnen 24 uur geleverd</div>
                        </div>
                        <div className="text-base md:text-lg font-bold text-red-600">+€50</div>
                      </label>
                    </div>
                  </div>
                )}
                ...
                {woningType && oppervlakte && (
                  <div className="text-center space-y-4 md:space-y-6">
                    ...
                    {berekendeprijs !== null && (
                      <div className="bg-lime-50 border-2 border-lime-200 rounded-xl p-4 md:p-6 mt-6">
                        <h4 className="text-xl md:text-2xl font-bold text-slate-800 mb-3 md:mb-4">Uw totale prijs:</h4>
                        <div className="text-3xl md:text-4xl font-bold text-lime-600 mb-3 md:mb-4">
                          €{berekendeprijs}
                        </div>
                        <div className="text-sm text-gray-600 mb-4 md:mb-6">Inclusief BTW</div>
      ...
      <footer className="relative py-12 px-4 bg-[#f4fcec]">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <Link href="/blogs" className="block text-slate-700 hover:text-lime-600 transition-colors">
                  • Blogs
                </Link>
              </div>
            </div>
          ...
          <div className="border-t border-slate-300 mt-8 pt-8 text-center">
            <p className="text-slate-700">© 2025 Label & Lens Amsterdam - Alle rechten voorbehouden</p>
          </div>
        </div>
      </footer>
    </div>
